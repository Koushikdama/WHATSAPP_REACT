      ...
      <div className="p-2 md:p-3">
        <div className="relative group">
          ...
          <input
            ...
            className="form-control form-control-sm w-full bg-[#202c33] border-transparent rounded-lg pl-10 pr-4 py-2.5 md:py-2 text-sm text-white focus:bg-[#2a3942] focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all duration-200"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
      </div>
      <div className="px-3 py-2 flex space-x-2 overflow-x-auto scrollbar-hide">
        {filters.map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`btn btn-sm rounded-full transition-colors border-0 whitespace-nowrap min-h-[44px] px-4 md:px-5 md:min-h-[36px] ${filter === f
              ? 'bg-primary text-white'
              : 'bg-[#202c33] text-gray-300 hover:bg-[#2a3942] active:bg-[#2a3942]'
              }`}
          >
            {f}
          </button>
