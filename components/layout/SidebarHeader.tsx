...
const SidebarHeader = () => {
  ...
  return (
    <>
      <header className="flex-shrink-0 bg-[#202c33] p-3 flex justify-between items-center">
        <AnimatedHeaderTitle />

        <div className="flex items-center space-x-4">
          {/* Notification Bell */}
          <Link to="/activity" className="text-gray-400 hover:text-gray-200 relative">
          ...
          {isDesktop && (
            <>
              <Link to="/new-chat" className="text-gray-400 hover:text-gray-200" title="New Chat">
                <MessagePlusIcon className="h-6 w-6" />
              </Link>
              <Link to="/create-group" className="text-gray-400 hover:text-gray-200" title="Create Group">
          ...
          <div className="relative" ref={menuRef}>
            <button onClick={() => setMenuOpen(!menuOpen)} className="text-gray-400 hover:text-gray-200">
              <ThreeDotsIcon className="h-6 w-6" />
            </button>
            {menuOpen && (
              <div className="absolute top-full right-0 mt-2 w-48 bg-[#233138] rounded-md shadow-lg z-20 py-1">
                <button className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-[#111b21]">New group</button>
                <button className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-[#111b21]">Starred messages</button>
                <Link to="/settings" className="block w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-[#111b21]" onClick={() => setMenuOpen(false)}>Settings</Link>
                <Link to="/settings/chats" className="block w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-[#111b21]" onClick={() => setMenuOpen(false)}>Chats Settings</Link>
                <button
                  onClick={handleLogout}
                  className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-[#111b21] font-medium"
                >
                  Log out
                </button>
              </div>
